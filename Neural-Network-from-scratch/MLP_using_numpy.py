...
class MLP(object):
    def __init__(self, eta=0.001, alpha=0.001, n_iter=10, batch_size=50):
    ...
    def add_bias_unit(self, layer, orientation):
        # Add bias unit to the specified layer
        if orientation == 'row':
    ...
    def predict(self, a4):
        # Make predictions based on the output of the network
        prediction = np.argmax(a4, axis=0)
        return prediction

    def compute_loss(self, prediction, label):
        # Compute the cross-entropy loss
        term_1 = -label * np.log(prediction)
    ...
    def compute_backward_pass(self, outputs, label):
        ...
        delta_4 = a4 - label # delta 4 is the output layer and a4 is the predicted output
        # Compute the error at the third layer (delta_3) using the weights from the output layer
        # and the sigmoid derivative of the third layer activations (sig_z3)
    ...
    def prep_data(self, X, y):
        # Prepare data by normalizing and creating mini-batches
        X_ = []
        y_ = []
    ...
    def fit(self, X, y):
        ...
        n_input = len(X[0, 0, :])  # Flattened image size (28*28 = 784)
        n_hidden_1, n_hidden_2, n_output = 100, 100, 10
        self.w1, self.w2, self.w3 = self.init_weights(n_input, n_hidden_1, n_hidden_2, n_output, self.batch_size)
        ...
        for i in range(self.n_iter):
            for j, (input, label) in enumerate(zip(X, y)):
                ...
                grad1, grad2, grad3 = self.compute_backward_pass([a1, z2, a2, z3, a3, z4, a4], one_hot_label)
            ...
            print('epoch', i, 'training accuracy =========== %.3f' % np.mean(np.matrix(train_acc)).item())

if __name__ == '__main__':
    # Create MLP instance and train on MNIST data
    mlp = MLP()
